...
async def get_by_username(
    db: AsyncSession, *, username: str, with_permissions: bool = False
) -> User | None:
...
async def count_active(db: AsyncSession) -> int:
    return await count_users(db, User.deleted_at.is_(None))


async def list_recent(db: AsyncSession, *, limit: int = 4) -> list[User]:
    result = await db.execute(
        select(User)
...
async def replace_user_roles(
    db: AsyncSession, user: User, *, role_ids: list[str]
) -> None:
    for ur in list(user.user_roles):
        await db.delete(ur)
    await db.flush()
    await add_user_roles(db, user_id=user.id, role_ids=role_ids)
